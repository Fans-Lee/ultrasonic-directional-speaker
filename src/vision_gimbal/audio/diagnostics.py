...
def format_diagnostics(record: dict) -> str:
    fields = (
        ...
        ("state", "state"),
        ("muted", "muted"),
        ("under", "underrun_count"),
        ("over", "overrun_count"),
        ("crc", "crc_error_count"),
        ("gap", "sequence_gap_count"),
        ("skip", "timer_skipped_samples"),
        ("host_drop", "host_tx_overrun_count"),
        ("capture_drop", "host_capture_overrun_count"),
        ("capture_restart", "host_capture_restart_count"),
        ("capture_age_ms", "host_capture_block_age_ms"),
        ("rx_error", "host_rx_error_count"),
        ("clip", "quantizer_clip_count"),
        ("tx_samples", "host_sent_samples"),
        ("tx_queue", "host_tx_queue_packets"),
        ("audio_age_ms", "last_audio_age_ms"),
        ("status_age_ms", "status_age_ms"),
    )
    text = " ".join(f"{label}={record[key]}" for label, key in fields)
    text += (
        f" buffer={record['buffer_fill_samples']}/{record['buffer_capacity_samples']}"
    )
